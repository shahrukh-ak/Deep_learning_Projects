...
input1 = genfromtxt('mnist_auto.txt', delimiter=',', usecols=(i for i in range(1,785)) , skip_header= 1)
...
x = tf.placeholder("float", [None, n_features]) ## (None, 784)
y_ = tf.placeholder("float", [None, n_features]) ## (None, 784)

###############################################################################
## nn with 1 hidden layer, neural net 4x3x4

def inference(x):
    Wh = tf.Variable( tf.random_uniform(  (784,184), -1.0 / math.sqrt(n_features), 1.0 / math.sqrt(n_features)   ) , dtype='float32' )
    bh = tf.Variable(   tf.zeros([184])  , dtype='float32'   )
    h1 = tf.nn.tanh( tf.matmul(x, Wh) +  bh   )

    #Wy = tf.transpose(Wh) #tied weights
    Wy = tf.Variable( tf.random_uniform( (184,784) ,  -1.0 / math.sqrt(n_features), 1.0 / math.sqrt(n_features)      )   , dtype='float32'   )
    by = tf.Variable(  tf.zeros([784])    , dtype='float32'    )
...
def loss_cross_entropy(y, y_):
    ...
    xentropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits=y, labels=y_)
