...
x_train = genfromtxt('times_train.csv', delimiter=',', usecols=(i for i in range(0,202)) , skip_header=1) 
y_train = genfromtxt('times_train.csv', delimiter=',', usecols=(0), skip_header=1)

x_test = genfromtxt('times_test.csv', delimiter=',', usecols=(i for i in range(0,202)) ,skip_header=1 )
y_test = genfromtxt('times_test.csv', delimiter=',', usecols=(0), skip_header=1)
...
B = 2 #len(sess.run(y_train_onehot[0]))
...
#def inference_nn(x, A, B):
    with tf.variable_scope("hidden_1"):
        hidden_1 = layer(x, [A, 10], [10] )
    with tf.variable_scope("output"):
        output = layer(hidden_1, [10,B], [B])
    return output

############################################################
## deep neural network 4 hidden layers

def inference_deep_4layers(x, A, B):
    with tf.variable_scope("hidden_1"):
        hidden_1 = layer(x, [A,30], [30])
    with tf.variable_scope("hidden_2"):
        hidden_2 = layer(hidden_1, [30,20], [20])
    with tf.variable_scope("hidden_3"):
        hidden_3 = layer(hidden_2, [20,10], [10])
    with tf.variable_scope("hidden_4"):
        hidden_4 = layer(hidden_3, [10,5], [5])
    with tf.variable_scope("output"):
        output = layer(hidden_4, [5, B], [B])
    return output
...
output = inference_deep_4layers(x, A, B)   ## deep nn
